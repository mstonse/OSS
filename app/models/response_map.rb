module Abstract
  def self.should_be_implemented
    mesg = "This class is not implemented"
...
=begin
  def import(row,session,id)
=end

  def self.import(row,session,id)

    index = settingIndexValue(row)
    assignment = Assignment.find(id)
    checkAssignmentNil(assignment,id)
    while index < row.length
      index = responseMapCheckOrCreation(assignment,row,id,index)
    end
  end
...
=begin
    if model == ReviewResponseMap
      new ReviewResponseMap.functionA1
    else
      new MetareviewResponseMap.functionA1
    end
=end
  #Abstract.should_be_implemented
    raise "settingIndexValue is not implemented in Response Map class"
  end

  def self.checkAssignmentNil(assignment,id)
=begin
     if model == ReviewResponseMap
       new ReviewResponseMap.functionB1(assignment,id)
     else
       new MetareviewResponseMap.functionB1(assignment,id)
    end
=end
    #Abstract.should_be_implemented
    raise "checkAssignmentNil is not implemented in Response Map class"
  end

  def self.responseMapCheckOrCreation(assignment,row,id,index)
=begin
    if model == ReviewResponseMap
      new ReviewResponseMap.functionC1(assignment,row,id,index)
    else
      new MetareviewResponseMap.functionC1(assignment,row,id,index)
    end
=end
   #Abstract.should_be_implemented
    raise "responseMapCheckOrCreation is not implemented in Response Map class"
  end
